...
def main_avinput_to_vcf(args):
    avinput_to_vcf(avinput=args.avinput, reference=args.reference, vcf=args.vcf)
...
def split_parser(subparsers):
    ...
    sub_parser.add_argument('--refgenes', '-r', nargs="+", help='refgene files')
    ...
    sub_parser.set_defaults(func=lambda args: split_annovar_by_gene(args.avoutput, refgenes=args.refgenes, gene_db=args.gene_db, outfile=args.outfile))
